...
class NewsController extends Controller
{
    ...
    // Show the dashboard for unauthorized users
    public function dashboardunauthorized()
    {
        $news = News::latest()->get(); // Removed user relationship
        return view('dashboardunauthorized', compact('news'));
    }
    ...
    public function store_news(Request $request)
    {
        ...
        // Create a new blog post and associate it with the user ID
        $news = new News();
        $news->headline = $request->headline;
        $news->content = $request->content;
        $news->author = $request->author;
        $news->date_published = $request->date_published;
        $news->user_id = 1; // Default user ID or set null if not needed
        $news->save();
